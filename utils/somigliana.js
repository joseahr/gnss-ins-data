...
function somigliana(latitude, helip) {
    ...
    var divisor = Math.sqrt(Math.pow(a, 2) * Math.pow(Math.cos(latitude), 2) + Math.pow(b, 2) * Math.pow(Math.sin(latitude), 2));
    ...
    var somiFull = somi * (1 - 2 / a * (1 + f + m + 2 * f * Math.pow(Math.sin(latitude), 2)) * helip + 3 / Math.pow(a, 2) * Math.pow(helip, 2)); // REVISAR ESTÁ MAL
